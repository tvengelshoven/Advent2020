...
def Group_Answer_Count(String):
    Set = String.split('\n')
    Answer_Counter = 0
    Count = 0
    Answers_Person_1 = Set[0]

    for Answer in Answers_Person_1:
        for i,Person in enumerate(Set):
            if Answer in Person:
                Answer_Counter += 1
        if Answer_Counter == len(Set):
            Count += 1
        Answer_Counter = 0
    return Count
...
Group_Sum = []
Group_Cumulative_Sum = []

for Group in Groups:
    Group_Total = Group_Count(Group)
    Group_Sum.append(Group_Total)
    Group_Cumulative= Group_Answer_Count(Group)
    Group_Cumulative_Sum.append(Group_Cumulative)

print('The sum of all group counts is:',sum(Group_Sum))
print('The sum of all cumulative answered questions is:',sum(Group_Cumulative_Sum))
